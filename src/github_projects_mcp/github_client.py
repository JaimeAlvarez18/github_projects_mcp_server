...
class GitHubClient:
    ...
    async def _get_user_ids(self, usernames: List[str]) -> List[str]:
        """Get GitHub user IDs from usernames.
        ...
        Raises:
            GitHubClientError: If any user is not found.
        """
        if not usernames:
            return []

        # Build a query to get multiple users at once
        user_queries = []
        variables = {}
        
        for i, username in enumerate(usernames):
            user_queries.append(f"user{i}: user(login: $login{i}) {{ id login }}")
            variables[f"login{i}"] = username

        query = f"""
        query GetUserIds({', '.join(f'$login{i}: String!' for i in range(len(usernames)))}) {{
          {' '.join(user_queries)}
        }}
        """
        ...
        try:
            ...
            for i, username in enumerate(usernames):
                user_data = result.get(f"user{i}")
                if not user_data:
                    raise GitHubClientError(f"User '{username}' not found")
                user_ids.append(user_data["id"])
                
            return user_ids
        except GitHubClientError as e:
            logger.error(f"Failed to get user IDs for {usernames}: {e}")
            raise

    async def create_issue(
        self, owner: str, repo: str, title: str, body: str = "", assignees: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        ...
        if assignees:
            try:
                assignee_ids = await self._get_user_ids(assignees)
            except GitHubClientError as e:
                logger.error(f"Failed to get assignee IDs: {e}")
                raise
    ...
    async def add_draft_issue_to_project(
        self, owner: str, project_number: int, title: str, body: str = "", assignees: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        ...
        Args:
            ...
            assignees: List of GitHub usernames to assign to the draft issue (optional)
                      Note: For draft issues, assignees are set through field updates after creation
        ...
        # Note: GitHub API doesn't support assignees directly in draft issue creation
        # Assignees would need to be set through field updates after creation
        if assignees:
            logger.warning(
                f"Assignees ({assignees}) provided for draft issue, but GitHub API doesn't support "
                "assignees in draft issue creation. Consider using update_project_item_field() "
                "after creation to set assignee fields."
            )
